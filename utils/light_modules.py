...
class LightningKWT(L.LightningModule):
    def __init__(self, config, useFnet=False):
        ...
        if self.config["mode"] == "multilabel":
            self.train_precision = AveragePrecision(
                task="multilabel", num_labels=self.num_classes
            )  # logging average precision
            self.val_precision = AveragePrecision(
                task="multilabel", num_labels=self.num_classes
            )  # logging average precision
            ...
            self.train_precision = MulticlassAccuracy(
                num_classes=self.num_classes
            )  # logging multiclass accuracy
            self.val_precision = MulticlassAccuracy(
                num_classes=self.num_classes
            )  # logging multiclass accuracy
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict(
            {"val_loss": val_loss}, on_epoch=True, on_step=True, sync_dist=True
        )
        return val_loss
...
class LightningSweep(L.LightningModule):
    def __init__(self, hparams, arch):
        super().__init__()
        self.model = KWT(**arch)
    ...
    def configure_optimizers(self):
        ...
        if self.scheduler:
            scheduler = get_cosine_schedule_with_warmup(
                self.optimizer, self.n_warmup, self.n_epochs
            )
            return [self.optimizer], [{"scheduler": scheduler, "interval": "epoch"}]
        else:
