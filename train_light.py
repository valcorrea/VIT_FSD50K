...
def get_dataloaders(extra_feats, config):
    ...
    train_set = SpectrogramDataset(
        manifest_path=config["tr_manifest_path"],
        labels_map=config["labels_map"],
        audio_config=config["audio_config"],
        augment=True,
    )
    val_set = SpectrogramDataset(
    ...
    train_loader = DataLoader(
        train_set, batch_size=config["hparams"]["batch_size"], num_workers=5
    )
    val_loader = DataLoader(
        val_set, batch_size=config["hparams"]["batch_size"], num_workers=5
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--tr_manifest_path",
        type=str,
        help="Path to the unlabeled train data manifest.",
    )
    ap.add_argument(
        "--val_manifest_path", type=str, help="Path to the unlabeled val data manifest."
    )
    ...
    ap.add_argument(
        "--dev_mode",
        action="store_true",
        help="Flag to limit the dataset for testing purposes.",
    )
